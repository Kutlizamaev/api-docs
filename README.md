...
- сохраняет UID аккаунта Bybit из request в `orders.merchant_id`;
...
Рекомендуемый вариант: передавать `bybitId` контрагента.

Backend найдет контрагента в базе по `bybitId`, возьмет его nickname и сохранит жалобу на этот nickname.
